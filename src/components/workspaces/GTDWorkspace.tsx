...
export function GTDWorkspace() {
  ...
  useEffect(() => {
    if (!user || gtdPages.length === 0) return;

    const loadTaggedNotes = async () => {
      try {
        // Load all Notes pages
  // Notes mode disabled: skip loading notes pages
  const notes: Page[] = [];
  ...
  <div className="w-72 bg-[#FBFBFA] dark:bg-gray-800 border-r border-gray-200/80 dark:border-gray-700 h-screen flex items-center justify-center">
          <div className="animate-pulse">
            ...
            <div className="space-y-2">
              <div className="h-10 w-48 bg-gray-200 dark:bg-gray-600 rounded"></div>
              <div className="h-10 w-48 bg-gray-200 dark:bg-gray-600 rounded"></div>
            </div>
          </div>
        </div>
        <div className="flex-1 bg-white dark:bg-gray-900 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400">Loading GTD workspace...</p>
          </div>
        </div>
      </div>
    );
  ...
  if (!user) {
    ...
    <div className="flex-1 flex items-center justify-center bg-white dark:bg-gray-900">
      ...
      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Authentication Required</h1>
      <p className="text-gray-600 dark:text-gray-400">Please log in to access your GTD workspace.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex h-screen w-full">
        ...
        <div className="flex-1 bg-white dark:bg-gray-900 overflow-y-auto">
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Select a GTD page</h3>
              <p className="text-gray-500 dark:text-gray-400">Choose a page from the sidebar to start organizing your tasks.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <GlobalDragProvider currentPageId={currentPageId || ''}>
      <div className="flex h-screen w-full relative">
        ...
        <button
          ...
          className="lg:hidden fixed top-4 left-4 z-50 p-2 bg-white dark:bg-gray-800 rounded-lg shadow-md min-h-[44px] min-w-[44px] flex items-center justify-center"
        >
        ...
        {/* Main Content */}
        <div className="flex-1 bg-white dark:bg-gray-900 overflow-y-auto pb-16 lg:pb-0">
          {activeView === 'smart' ? (
            <SmartView />
